...
(function () {
    ...
    *   on('command', { from: STRING, role: ["owner"|"moderator"|"viewer"|"guest"], command: STRING, params: ARRAY })
    *
    ******************************************************/
        ...
        var self = this, LiveCodingTV,
            ...
            applyCommands = function (data) {
                return new Promise((resolve, reject) => {
                    ...
                    if (data.message.indexOf('!') === 0 && data.message.split(1, 2) !== ' ') {
                        // Emit command before using widgets
                        let cmdData = { from: data.from, role: data.role, command: data.message.toLowerCase().slice(1).split(' ')[0], params: data.message.toLowerCase().split(' ').slice(1) };
                        emitCommand(cmdData);
